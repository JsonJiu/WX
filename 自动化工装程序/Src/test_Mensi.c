...
void Test_Function_Mensi(void)
{
	...
	case t_mode_select:
		interval_timing = 0;
		single_step_timing = 10000;
		if (Test_result.module_type == none)
		{
			memset(Test_result.LoraEUI, 0x00, 16);
			Test_process = t_self_check;
		}
		if (Test_result.module_type == mensi_lora)
		{
			Test_process = t_close_commumication;
		}
		break;
	case t_close_commumication:
		if (single_step_timing > 0)
		{
			if (Test_agreement_receive == r_close_commumication)
			{
				Test_agreement_receive = NO_receive;
				interval_timing = 0;
				single_step_timing = 10000;
	...
	case t_find_report:
		if (single_step_timing > 0)
		{
			if (Test_agreement_receive == r_find_report)
			{
				Test_agreement_receive = NO_receive;
				interval_timing = 0;
				single_step_timing = 10000;
		...
		if (single_step_timing == 0 && repeat_report < 3 && Test_agreement_receive == r_find_report_ERR)
		{
			repeat_report++;
			Test_agreement_receive = NO_receive;
			single_step_timing = 10000;
	...
	case t_self_check:
		if (single_step_timing > 0)
		{
			if (Test_agreement_receive == r_self_check)
			{
				Test_agreement_receive = NO_receive;
