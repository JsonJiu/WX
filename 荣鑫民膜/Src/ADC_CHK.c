...
static void MF_ADC_Common_Init(void) {
  ...
  Common_InitStruct.clockSource =
      FL_CMU_ADC_CLK_SOURCE_RCHF; /*配置ADC工作时钟源*/
  Common_InitStruct.clockPrescaler =
      FL_ADC_CLK_PSC_DIV8; /*配置ADC工作时钟分配*/
...
//   (void)FL_GPIO_Init(GPIOD, &GPIO_InitStruct); /*GPIO初始化*/
  ...
  ADC_InitStruct.autoMode =
      FL_ADC_SINGLE_CONV_MODE_AUTO; /*配置ADC转换流程，仅对单次转换有效*/
  ADC_InitStruct.waitMode = FL_ENABLE;    /*配置ADC等待模式*/
  ADC_InitStruct.overrunMode = FL_ENABLE; /*配置ADC_Overrun模式*/
  ADC_InitStruct.scanDirection =
      FL_ADC_SEQ_SCAN_DIR_FORWARD; /*配置ADC扫描顺序*/
  ADC_InitStruct.externalTrigConv =
      FL_ADC_TRIGGER_EDGE_NONE; /*配置非软件触发使能及极性*/
  ADC_InitStruct.triggerSource = FL_DISABLE; /*配置ADC非软件触发源*/
  ADC_InitStruct.fastChannelTime =
      FL_ADC_FAST_CH_SAMPLING_TIME_32_ADCCLK; /*配置ADC快速通道采样时间*/
  ADC_InitStruct.lowChannelTime =
      FL_ADC_SLOW_CH_SAMPLING_TIME_512_ADCCLK; /*配置ADC慢速通道采样时间*/
  ADC_InitStruct.oversamplingMode = FL_DISABLE; /*配置ADC过采样模式*/
  ADC_InitStruct.overSampingMultiplier =
      FL_ADC_OVERSAMPLING_MUL_16X; /*配置ADC过采样率*/
  ADC_InitStruct.oversamplingShift =
      FL_ADC_OVERSAMPLING_SHIFT_4B; /*配置ADC过采样结果移位*/
...
static uint8_t GetSingleChannelSample_POLL(uint32_t channel,
...
uint32_t GetSingleChannelVoltage_POLL(uint32_t channel) {
  ...
  if ((Get122VSample != 0) && (Get122VSample_State == 0) &&
      (GetVSample_State == 0)) {
    GetChannelVoltage =
        (uint32_t)(((uint64_t)GetVSample * 3000 * (ADC_VREF)) /
                   ((uint64_t)Get122VSample * 4095)); // 计算通道电压
  }

  return GetChannelVoltage;
}
// 获取主电位置的电压
uint32_t get_zhudian_weizhi_dianya() {
  uint32_t test_shuju = 0;

  test_shuju =
      GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH7); // 主电ADC通道: ADC_IN8
...
// 获取备电电源电压
uint32_t get_NB_weizhi_dianya() {
  uint32_t test_shuju = 0;
  erji_dianya_CHK_CTRL_ON();
...
// }
// // 主电供电的电压
// uint32_t get_zhukongban_gongdian_dianya() {
//   uint32_t test_shuju = 0;
...
// // ADC IN1 - 3.3V电压检测 (直连ADC，无分压)
// uint32_t get_adc_in1_voltage(void) {
//   uint32_t test_shuju = 0;
//   test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH1);
//   // 注意：3.3V直连ADC，不需要乘分压系数
//   return test_shuju;
// }
// // 工装的供电电压 (5V)
// uint32_t get_gongzhuang_gongdian_dianya() {
//   uint32_t test_shuju = 0;
//   // 使能工装供电检测,开启PE0
//   zhudian_dianya_CHK_CTRL_ON();
//   FL_DelayMs(5); // 等待电压稳定
...
// ADC IN8 电压读取
// PD2 - 用于检测板上5V电压
uint32_t get_adc_in8_voltage(void) {
  uint32_t test_shuju = 0;
  test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH8);
  // 5V，经过分压电路，需要乘以分压系数11
  test_shuju = test_shuju * 11;
  return test_shuju;
}
...
// ADC IN9 电压读取 ( 3.3V)
// PD4 - 用于检测产品的3.3V电压
uint32_t get_adc_in9_voltage(void) {
  uint32_t test_shuju = 0;
...
// ADC IN2 电压读取 (RTC电池电压)
// PD3 - 用于检测RTC电池电压  (5V经二极管后约4.7V)
uint32_t get_adc_in2_voltage(void) {
  uint32_t test_shuju = 0;
