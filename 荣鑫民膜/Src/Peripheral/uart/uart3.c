#define LOG_TAG "uart3"
#include "Peripheral/uart/uart3.h"
#include "LED_CTRL.h"
...
void UART3_IRQHandler(void) {
  ...
  if ((UART3RXBuffFullIT == 0x01UL) && (UART3RXBuffFullFlag == 0x01UL)) {
    // 中断接收
    uart3_Rec_shuju_neirong[uart3_Rec_shuju_neirong_count] =
        FL_UART_ReadRXBuff(UART3); // 接收中断标志可通过读取rxreg寄存器清除
    uart3_Rec_shuju_neirong_count++;
    if (uart3_Rec_shuju_neirong_count >= 200) {
      uart3_Rec_shuju_neirong_count = 0;
    }
    uart3_Rec_shuju_flag = 1;
    uart3_Rec_shuju_time_count = 100;
  }
...
void Uart3_Rx_rec() {
  if (uart3_Rec_shuju_flag == 1 && uart3_Rec_shuju_time_count == 0) {
    ...
    // 对返回数据进行解析
    memcpy(uart3_Uart0_Tx_SendData, uart3_Rec_shuju_neirong,
           uart3_Rec_shuju_neirong_count);
    log_d("当前Uart3计量模块接收数据:");
    protocol_debug_print(uart3_Uart0_Tx_SendData,
                         uart3_Rec_shuju_neirong_count);
    JILIAN_TONGXIN_xieyijiexi(uart3_Uart0_Tx_SendData,
                              uart3_Rec_shuju_neirong_count);
    // Uart3_Tx_Send(uart3_Uart0_Tx_SendData,uart3_Rec_shuju_neirong_count);
    // Bug4修复: 清空接收缓冲区，防止脏数据残留导致协议解析错误
    memset(uart3_Rec_shuju_neirong, 0, sizeof(uart3_Rec_shuju_neirong));
    uart3_Rec_shuju_neirong_count = 0;
  }
}
