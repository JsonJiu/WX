#include "Peripheral/uart/uart4.h"
...
uint8_t qiandao5[] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
...
void UART4_IRQHandler(void) {
  ...
  if ((UART4RXBuffFullIT == 0x01UL) && (UART4RXBuffFullFlag == 0x01UL)) {
    // 中断接收
    uart4_Rec_shuju_neirong[uart4_Rec_shuju_neirong_count] =
        FL_UART_ReadRXBuff(UART4); // 接收中断标志可通过读取rxreg寄存器清除
    uart4_Rec_shuju_neirong_count++;
    if (uart4_Rec_shuju_neirong_count >= 200) {
      uart4_Rec_shuju_neirong_count = 0;
    }
    uart4_Rec_shuju_flag = 1;
    uart4_Rec_shuju_time_count = 100;
  }
...
void Uart4_Tx_Send_ok(uint8_t zufuchua[], uint16_t lenth) {
  ...
  if (qiandao_biaoshi_4 == 1) {
    for (diandao_xunhuan = 0; diandao_xunhuan < 32; diandao_xunhuan++) {
      // 这里会有长时间阻塞，仍个看门狗
#ifdef ENABLE_WATCHDOG
      FL_IWDT_ReloadCounter(IWDT);
#endif
      // Uart4_Tx_Send(qiandao, 50);
      Uart4_Tx_Send(qiandao, 12);
    }
...
void Uart4_Rx_rec() {
  if (uart4_Rec_shuju_flag == 1 && uart4_Rec_shuju_time_count == 0) {
    ...
    // 对返回数据进行解析
    memcpy(uart4_Uart0_Tx_SendData, uart4_Rec_shuju_neirong,
           uart4_Rec_shuju_neirong_count);
