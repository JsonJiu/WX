...
void Current_CHK_CTRL_ON_l()
{
	...
	{
		case 0:
			Current_7V2_I_CHK_CTL(On);
		break;
		case 1:
			Current_3V6_I_CHK_CTL(On);
		break;
		default:
			break;
	}
...
uint16_t Current_CHK_Func(uint8_t gongwei) //0-7V2  1-3V6
{
	uint16_t dianliu;
	uint16_t dianliu1;
	...
	dianliu = dianliu1;
	if(gongwei == 0)
		dianliu-=26;   //INA219芯片会受采样电阻端的电压影响，是测电压7.2V时，工装测试板自身功耗为28uA左右
	return dianliu;
}
