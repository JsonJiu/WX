...
*/
uint8_t Start_Shanggao_Agreement[24] = {0x68,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x68,0x01,0x0B,0x00,0x25,0x06,0x19,0x16,0x24,0x04,0x02,0x05,0x10,0x00,0x03,0x72,0x16};
	/*————————A027打开星闪或蓝牙协议 数据码A027————————*/
/*
发送打开星闪蓝牙，因为测功耗的时候会把星闪关掉
*/
uint8_t OPEN_XINGSHAN[24] = {0x68,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x68,0x04,0x0B,0x00,0x25,0x06,0x18,0x14,0x31,0x52,0x02,0x27,0xA0,0x00,0x01,0x7D,0x16};
			...
			if(sum_jiaoyan == sum_jiaoyan_jisuan)
			{
				...
				switch(Data_sign)
				{
					...
					case 0xE20A:
						...
						Test_result.flash = Data[pHead+32];
						if(Data[pHead+35]==1&&Data[pHead+36]==1)
						{
						Test_result.PT=1;
						}
						//mes没有蓝牙检查项 用led状态代替
						Test_result.lanya=Data[pHead+37];
						Test_result.LED=Test_result.lanya;
						Test_result.KEY_OPEN=1-Data[pHead+38];
						Test_result.Alarm119=Data[pHead+39];
...
/*————————打开星闪蓝牙————————*/
void Open_xingshan(void)
{
	Send_Qiandao();
	Uart1_Tx_Send(OPEN_XINGSHAN,24);
}
