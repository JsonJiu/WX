...
extern uint16_t anjian_xinhuan_guanbi;
//开始测试设置成功后发送
...
//查询结果
void PC_xieyifasong_3(uint8_t zufuchua[],uint16_t lenth)
{
	...
	memset(xieyi2_fanhui,0x00,send_lenth);
	xieyi2_fanhui[jishu_lenth++] = 0x68;
	xieyi2_fanhui[jishu_lenth++] = 0xAF;
	xieyi2_fanhui[jishu_lenth++] = 0x00;
	xieyi2_fanhui[jishu_lenth++] = Test_jiejuo_jilu.gongwei;
	memcpy(&xieyi2_fanhui[jishu_lenth],zufuchua,lenth);
	jishu_lenth+=lenth;
	//对帧长度赋值
	xieyi2_fanhui[2] = jishu_lenth+2;
	xieyi2_fanhui[jishu_lenth] = 0;
...
void PC_xieyijiexi(uint8_t zufuchua[],uint16_t lenth)
{
	...
	while(1)
	{
		...
		if(zufuchua[pHead]==0x68)
		{
			...
			else if(zufuchua[pHead+1]==0xAE&&zufuchua[pHead+3]==Test_jiejuo_jilu.gongwei&&zufuchua[pHead+4]==0x68)
			{
				if(zufuchua[pHead+1]==0xAE)
				{
					//获取协议长度
					zhenchangdu = zufuchua[pHead+2];
					//长度应低于100个字节
					if(zhenchangdu<100)
					{
						...
						if(zufuchua[pHead+3]==Test_jiejuo_jilu.gongwei&&zufuchua[pHead+zhenchangdu-1]==0x16)
						{
							...
							if(hejiaoyan==zufuchua[pHead+zhenchangdu-2])
							{
								//进入透传膜式
								touchuan_flag = 1;
								if(Test_liucheng_L==w_wait)
								{
									//按键触发红外
									anjian_kongzhi_ON();
								}
								Uart1_Tx_Send_ok(&zufuchua[pHead+4],zhenchangdu-6);
								pHead+=zhenchangdu;
							}
						}
					}
