...
CHECKPRESULT_S CheckResult = {
                                ...
                                0xFF,           //保留
                                0xFFF0,           //状态
                                0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,//IMEI
                                0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,//IMSI
                                0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,0x38,//ICCID
                                0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,//ESAM
                                0xFF,
                                };
...
void Process_AutoCheck(void)
{
    ...
    //IrdaUartDataFormFrame(OPT_WRITE, DEV_AutoCheckFinish);
    switch(DeviceTool.StatusCheck)
    {
        case chk_Halt:   //在未检测到命令的时候，空转
            ...
            break;
        case chk_Start:  //开始检测
             Debug(DeviceTool.FlagMasterOnLineVolt);
...
//                    DeviceTool.StatusCheck = chk_end;
                    ...
                    memset(&CheckResult.MeterTYP, 0x00, sizeof(CHECKPRESULT_S) - 1);   //表具类型清零？
...
//                SetUartTxDirIn();
            ...
            //增加上告查询机制,每五秒检测一次  wxf
            if(DeviceTool.SecondsTiming%5==0)
            {
            IrdaUartDataFormFrame(OPT_WRITE, DEV_AutoCheckFinish);
            //DeviceTool.FlagSetHightLow = 2;//告知主控板的输出IO口，输出低电平
             //   DelayMs(200);
             //   IrdaUartDataFormFrame(OPT_WRITE, DEV_SETOUTIOSTATUS);
            }
            break;
...
/*****************************红外通讯****************************************/
        ...
        case chk_DeviceToolConnectMasterBoard:
            if(DeviceTool.FlagCheckProcess == MASTER_CONNCET_CHECK)//若读取到返回或是超时
            {
                ...
                DeviceTool.CountReapeat = 0;
                DeviceTool.SecondsTiming = 0;
                DeviceTool.StatusCheck = chk_find_ICCID;  //调试
                Debug(0x22);
...
//                    DeviceTool.StatusCheck = chk_end;
                    ...
                    memset(&CheckResult.MeterTYP, 0x00, sizeof(CHECKPRESULT_S) - 1);
                    OM_SendUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n========Connect OverTime!=========\r\n"));
                }
                else                                 //第一次检测失败，再发送一次
                {
...
//这部分暂时屏蔽掉
        case chk_find_IMEI:
            ...
            {
                DeviceTool.SecondsTiming = 5; //将延时增加3秒
                DeviceTool.CountReapeat++;
                if(DeviceTool.CountReapeat<=3)
                {
                MeterUartDataFormFrame(OPT_READ, DEV_read_IMEI_IMSI);//发送协议以读取IMEI
                OM_SendUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n========读取IMEI=========\r\n"));
                }
            }
            if(DeviceTool.FlagCheckProcess == MASTER_IMEI_CLOSED||DeviceTool.CountReapeat>3)
            {
              DeviceTool.CountReapeat = 0;
              DeviceTool.SecondsTiming = 0;
              DeviceTool.StatusCheck = chk_find_ESAM;
            }
            break;
            case chk_find_ICCID:
            if(DeviceTool.SecondsTiming==0)
            {
                ...
                if(DeviceTool.CountReapeat<=3)
                {
                MeterUartDataFormFrame(OPT_READ, DEV_read_ICCID);//发送协议以读取IMEI
                OM_SendUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n========读取IMEI=========\r\n"));
                }
            }
            if(DeviceTool.FlagCheckProcess == MASTER_ICCID_CLOSED||DeviceTool.CountReapeat>3)
            {
              DeviceTool.CountReapeat = 0;
              DeviceTool.SecondsTiming = 0;
              DeviceTool.StatusCheck = chk_find_ESAM;
        ...
        case chk_find_ESAM:
            if(DeviceTool.SecondsTiming==0)
            {
                ...
                if(DeviceTool.CountReapeat<=3)
                {
                MeterUartDataFormFrame(OPT_READ, DEV_read_ESAMID);//发送协议以读取ESAM
                OM_SendUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n========读取ESAM=========\r\n"));
                }
                
            }
            if(DeviceTool.FlagCheckProcess == MASTER_ESAM_CLOSED||DeviceTool.CountReapeat>3)
            {
                DeviceTool.CountReapeat = 0;
                DeviceTool.SecondsTiming = 0;
        ...
        case chk_SetMsterBoardOutLow:
            if(DeviceTool.SecondsTiming)
            {
                if(DeviceTool.FlagCheckProcess == MASTER_CHECK_TWO)
                {
                    ...
                    DeviceTool.StatusCheck = chk_SetMsterBoardOutRest;
                    Debug(0x42);
//                    LedOrBeepTick((LED_TypeDef)(GREEN_LED) , TICK_COUNT(8), TICK_ON_1MS(10), TICK_OFF_1MS(80));
                }
            }
            ...
            break;
            case chk_SetMsterBoardOutRest:
            if(DeviceTool.SecondsTiming)
            {
                if(DeviceTool.FlagCheckProcess == MASTER_CHECK_THREE)
                {
                    DeviceTool.CountReapeat = 0;
                    DeviceTool.SecondsTiming = 0;
                    OM_SendUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n== Check Low Finish ===\r\n"));
...
//                    LedOrBeepTick((LED_TypeDef)(GREEN_LED) , TICK_COUNT(8), TICK_ON_1MS(10), TICK_OFF_1MS(80));
        ...
        case chk_MasterBoard_CloseIR:   //关闭红外实质上在上一步已经完成了 这一步我不知道究竟发生了什么，应该无法进入else中。
            if(DeviceTool.SecondsTiming)
            {
                if(DeviceTool.FlagCheckProcess == MASTER_IR_CLOSED)
                {
                    ...
                    DeviceTool.SecondsTiming = 6; //将延时减少3秒
            ...
            else
            {
                ...
                if(DeviceTool.CountReapeat > 2 )
                {
                    ...
                    SetMasterBoardPower(DISABLE);//断开主控板电源
                    DeviceTool.CountReapeat = 0;
                    DeviceTool.SecondsTiming = 6; //减少3
                    DeviceTool.StatusCheck = chk_CheckCurrent;
                    OM_SendUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n== IR Close Over Time ===\r\n"));
                    DelayMs(3000);//增加延时
                }
...
//                ndUsartData(Debug_COM, DebugBuf, sprintf((char*)DebugBuf,"\r\n==结果已出可查询==\r\n")); // add by mjl
              }
            }
             */
//            if(CheckResult.DeviceID == IDNUM)
...
/* 设置主控板IO输出 */
unsigned char SetMasterOutRest(unsigned char *data, unsigned char outtype)
{
    unsigned char len = 1;
#ifdef    DEF_VALVE_CHECK
    data[len++] = IO_VALVE; data[len++] = 0x02;//停止阀门动作
#endif
...
/*接收读取IMEI以及IMSI的结果**/
unsigned char FindICCID_where(unsigned char *data)
{
    unsigned char len = 0;
    memset(CheckResult.FindIMEI,0x00,20);
    memcpy(CheckResult.FindIMEI,&data[len] , 15);
    len+=15;
    memset(CheckResult.FindIMSI,0x00,20);
    memcpy(CheckResult.FindIMSI,&data[len], 15);
    len+=15;
    memset(CheckResult.FindICCID,0x00,20);
    memcpy(CheckResult.FindICCID,&data[len], 20);
    len+=20;
    len += 46;
    memcpy(CheckResult.SoftVertime,&data[len] ,6);
    //CheckResult.charkbat = 0x00;
    return len;
}

unsigned char DEV_chick_AutoCheckFinish(unsigned char *data)
{
    unsigned char len = 0;
    unsigned char jieguo = 0;
    len++;
    if(data[len]<=33&&data[len]>0)//上告完成
    {
        jieguo = 1;
    }
    //CheckResult.charkbat = 0x00;
...
unsigned char Find_ESAMID_where(unsigned char *data)
{
    unsigned char len = 0;
    len+=32;//表具编号
    len+=1;//加密密钥长度
    ...
    len+=3;//保留
    len+=15;//IMEI号
    memcpy(CheckResult.FindESAMID,&data[len], 8);
    //CheckResult.charkbat = 0x00;
    return len;
}
