...
void UART1_IRQHandler(void) {
  ...
  if ((UART1RXBuffFullIT == 0x01UL) && (UART1RXBuffFullFlag == 0x01UL)) {
    // 中断接收
    uart1_Rec_shuju_neirong[uart1_Rec_shuju_neirong_count] =
        FL_UART_ReadRXBuff(UART1); // 接收中断标志可通过读取rxreg寄存器清除
...
void Uart1_Tx_Send_ok(uint8_t zufuchua[], uint16_t lenth) {
  ...
  if (qiandao_biaoshi == 1) {
    for (diandao_xunhuan = 0; diandao_xunhuan < 40; diandao_xunhuan++) {
      // 这里会有长时间阻塞，仍个看门狗
#if WATCH_DOG_ENABLE
    ...
    Uart1_Tx_Send(qiandao_jiaozhun, 5);
  }
...
void Uart1_Rx_rec() {
  if (uart1_Rec_shuju_flag == 1 && uart1_Rec_shuju_time_count == 0) {
    ...
    // 对返回数据进行解析
    memcpy(uart1_Uart0_Tx_SendData, uart1_Rec_shuju_neirong,
           uart1_Rec_shuju_neirong_count);
    // 直接打印实际接收字节数
    DeBug_print("[RX=%d] ", uart1_Rec_shuju_neirong_count);
    protocol_debug_print(uart1_Uart0_Tx_SendData,
                         uart1_Rec_shuju_neirong_count);
