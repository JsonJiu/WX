...
extern uint16_t ceshi_diancifa_yanshi;
extern uint16_t debug_print_time;
extern uint16_t famen_debug_print_time;
...
void ATIM_IRQHandler() {
  if (FL_ATIM_IsEnabledIT_Update(ATIM) && FL_ATIM_IsActiveFlag_Update(ATIM)) {
    ...
    if (ceshi_diancifa_yanshi > 0) {
      ceshi_diancifa_yanshi--;
    }
    if (debug_print_time > 0 && Debug_Mode == 1) {
      debug_print_time--;
    }
    if (famen_debug_print_time > 0 && Debug_Mode == 1) {
      famen_debug_print_time--;
    }
    // 更新系统毫秒计数器
    system_tick_ms++;
  }
