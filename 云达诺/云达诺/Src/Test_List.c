...
uint16_t jiou_jishu = 0;
uint32_t jiliang_maichongjishu = 0;
...
//					SENSOR_P_Low();
//					SENSOR_D_Low();
//					SENSOR_T_High();   //因为水表板需要中断才会触发，所以要先改变一下电平
//				}
			}
			else
			{
		...
		case w_famen_guan:
	      ...
	      Test_quanju_canshu_L.danbu_chaoshishijian_ms = 20000;
			...
			{
				test_xieyi_jilu_Rec = No_Receive;
				if(Test_jiejuo_jilu.jiliang_fangshi == youci && Test_linshi_cunchushuju_L.L_youcijiliang>0)
				{
...
//					}
					...
					{
					Test_liucheng_L = w_zhudong_shangg2; 
					}
					if(Test_jiejuo_jilu.FM_Status == wufa) //如果无阀
					{
				...
				if(test_xieyi_jilu_Rec == w_get_shanggao)
			{
				...
				Test_quanju_canshu_L.time_softdelay_ms = 2000;
				//重置单步超时时间 20秒
	      Test_quanju_canshu_L.danbu_chaoshishijian_ms = 25000;
				Test_liucheng_L = w_famen_kai;
				}
			else
			{
