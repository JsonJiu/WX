...
//获取NB电源电压2
uint32_t get_NB2_weizhi_dianya()
{
	uint32_t test_shuju = 0;
	NB_dianya_duqu_set_Hi();
...
{
	uint32_t test_shuju = 0;
	FM_fuji_dianyuan_gongdian_On();
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH16);
	test_shuju = test_shuju*11;
...
{
	uint32_t test_shuju = 0;
	FM_zhengji_dianyuan_gongdian_On();
	test_shuju = GetSingleChannelVoltage_POLL(FL_ADC_EXTERNAL_CH17);
	test_shuju = test_shuju*11;
	FM_zhengji_dianyuan_gongdian_Off();
	return test_shuju;
}
//读取主控电压
uint32_t get_zhukong_dianya()
{
	uint32_t test_shuju = 0;
	VDD_dianya_duqu_set_Hi();
...
//119供电的电压
//PD6
uint32_t get_119_gongdian_dianya()
{
	uint32_t test_shuju = 0;
	V119_dianya_duqu_set_Hi();
