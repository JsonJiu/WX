...
unsigned char TaskNum_IR_UartRecv_OverTime = 0;
 unsigned char testok[20]="test ok?\rtest ok\r";
    ...
    while(RecvBuf->p_RxDataWrite != RecvBuf->p_RxDataRead)
    {
    	...
    	if(RecvBuf->f_RecvCmdHead == 0)
    	{
            if(dat == 0x68)//威星协议
            {
             ...
             else if(dat == 0x74)//test ok
            {
                 RecvBuf->f_RecvCmdHead = 2;
                RecvBuf->RecvCmdBuf[RecvBuf->p_RecvCmdData] = dat;
                RecvBuf->p_RecvCmdData = 1;
    	...
    	{
            if(RecvBuf->f_RecvCmdHead == 2)//74 65 73 74 20 6F 6B 3F 0D 74 65 73 74 20 6F 6B 0D
        {
                if(dat==testok[RecvBuf->p_RecvCmdData])
                {
            RecvBuf->p_RecvCmdData++ ;
            if(RecvBuf->p_RecvCmdData== 4)
            {
                RecvBuf->f_RecvCmdHead = 0;
                 return test_finish;
            }
