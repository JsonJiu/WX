...
void Process_AutoCheck(void)
{
    ...
    switch(DeviceTool.StatusCheck)
    {
        ...
        case chk_MasterOnLine:
            if((DeviceTool.FlagMasterBoardSelfCheckFinish == MASTER_SELFCHECK_FINISH)||xhqd==1 || (DeviceTool.SecondsTiming == 0))//等待主控板自检完成
            {
...
//                    DeviceTool.StatusCheck = chk_end;
        ...
        case chk_CheckCurrent:                        //检测电流
            if(DeviceTool.SecondsTiming == 0)
            {
                ...
                hall_chick_Reset();//进入IO口检测之前先重置霍尔检测
#endif
...
unsigned char RecvConnectResult(unsigned char *data)
{
    ...
    CheckResult.Status.Morden = data[len++];
     if(xhqd==1)
     {
         CheckResult.ValueSingal = 30;
         memset(CheckResult.FindIMEI,0x38, 20);
    memset(CheckResult.FindIMSI,0x38, 20);
     }//data[len++];
     else
     {
         CheckResult.ValueSingal = 255;
         memset(CheckResult.FindIMEI,0x30, 20);
    memset(CheckResult.FindIMSI,0x30, 20);
     }//data[len++];
    len++;
...
unsigned char FindIMEIandIMSIandICCID_where(unsigned char *data)
{
   ...
   // memcpy(CheckResult.FindICCID,&data[offset] ,20);
    ...
    //memset(CheckResult.ESAMID, 0x38, 8);
    //memset(CheckResult.BANZAI, 0x38, 12);
}
...
unsigned char hall_A_1,hall_B_1,hall_A_2,hall_B_2=0;
/* 获取主控板输入IO口的状态 */
...
//        CheckResult.Status.Valve = 0;//关到位
    ...
    if(intype == 1)
    {
        hall_A_1 = data[len++];//  01 01 vv 01 00   n  //  00 01 vv 01 00  y
        hall_B_1 = data[len++];
    }
    else//这里应该是第二次测试，第二次测试会保留结果
    {
        hall_A_2 = data[len++];
        hall_B_2 = data[len++];       
        if((hall_A_1+hall_A_2)==1&&(hall_B_1+hall_B_2)==1)
        {
        }
        else
        {
