...
void Test_Function(void)
{
	...
	switch(Test_process)
	{
		...
		case t_get_7V2_voltage:  //7.2V电压输出正常才会开始检测
			...
			if(Test_result.Voltage_7V2_Mould > 7000 && Test_result.Voltage_7V2_Mould < 7600)
			{
					Test_process = t_close_screen_led;
					single_step_timing = 20000;//10s时间关闭
			}
			else
				Test_process = t_end;
			break;
	  ...
	  case t_IC_test:  //刷卡得放在上告前，貌似上告后刷卡没反应
		...
		case t_start_shanggao:
			if(single_step_timing>0)
			{
				...
				{
					Test_agreement_receive = NO_receive;
					single_step_timing = 20000;//10串口模拟上告发送时间
