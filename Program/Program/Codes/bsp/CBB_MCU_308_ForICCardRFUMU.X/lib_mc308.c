...
IRQServerFT IRQServerCN42 = IrqServerDefault;
...
unsigned char ReadConfigWordQ[7] = { 0xFF, 0x56, 0xAE, 0x35, 0xA9, 0x55, 0xF0 };
void __attribute__((interrupt, no_auto_psv)) _CNInterrupt(void)
{
    InputChange_Clear_Intr_Status_Bit;
    if ((CNEN3bits.CN42IE == 1) && (PORTAbits.RA10 == 0))
        IRQServerCN42();
...
void MCUPllCfg(void)
{
    ...
    OSCCONbits.NOSC = 0;
    _OSWEN = 0;//?Â¹??
    _SOSCEN = 1;//32 kHz Â¸Â¨?Ãº?Ã±ÂµÂ´?Ã·Â£Â¨SOSCÂ£Â©?Â¹???Â»-?Â¹??Â¸Â¨?Ãº?Ã±ÂµÂ´?Ã·
    _POSCEN = 0;//?Ã·?Ã±ÂµÂ´?Ã·???????Â£?Â½??Â¼Ã¤Â±Â»Â½Ã»?Â¹
