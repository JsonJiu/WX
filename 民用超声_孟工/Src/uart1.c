...
#include "ir.h"
#include "test.h"
#define lenth_Receive_Send_MAX 800
...
uint8_t qiandao_jiaozhun[] = {0xFE, 0xFE, 0xFE, 0xFE, 0xFE};
// 这个带前�?
...
void UART1_IRQHandler(void)
{
    ...
    {
        // �?�?接收
        uart1_Rec_shuju_neirong[uart1_Rec_shuju_neirong_count] = FL_UART_ReadRXBuff(UART1); // 接收�?�?标志�?通过读取rxreg寄存器清�?
        ...
        uart1_Rec_shuju_time_count = 200; // 判断串口空闲状态最长时�?100ms
    }

    // 发送中�?处理
    if ((UART1TXBuffFullIT == 0x01UL) && (UART1TXBuffFullFlag == 0x01UL))
    {
        // 这里发送完成之后再进进入，�?以解决最后一�?字节发不完的�?�?
        if (UART1Op.TxOpc == UART1Op.TxLen)
        {
            FL_UART_DisableIT_TXShiftBuffEmpty(UART1);
            send_over_flag1 = 1;
        }
        // 发送中�?标志�?通过写txreg寄存器清除或txif�?1清除
        // 发送指定长度的数据
        if (UART1Op.TxOpc < UART1Op.TxLen)
        {
            FL_UART_WriteTXBuff(UART1, UART1Op.TxBuf[UART1Op.TxOpc]); // 发送一�?数据
            UART1Op.TxOpc++;
        }

        FL_UART_ClearFlag_TXShiftBuffEmpty(UART1); // 清除发送中�?标志
    }
}
...
void Uart1_Tx_Send_ok(uint8_t zufuchua[], uint16_t lenth)
{
    ...
    {
        for (diandao_xunhuan = 0; diandao_xunhuan < 24; diandao_xunhuan++)
        {
            // 这里会有长时间阻塞，仍个看门�?
            FL_IWDT_ReloadCounter(IWDT);
            Uart1_Tx_Send(qiandao, 50);
        }
    }
...
void Uart1_Rx_rec()
{
    if (uart1_Rec_shuju_flag == 1 && uart1_Rec_shuju_time_count == 0)
    {
        ...
        IR_Agreement_Analysis(uart1_Uart0_Tx_SendData, uart1_Rec_shuju_neirong_count);
        uart1_Rec_shuju_neirong_count = 0;
    }
}
