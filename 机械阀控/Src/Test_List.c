...
//			break;
			...
			if(Test_linshi_cunchushuju_L.L_daowei1 == 0 && Test_linshi_cunchushuju_L.L_daowei2 == 1)
					{
						...
						//第一次无磁计量，读取无磁计量值
			if(Test_jiejuo_jilu.jiliang_fangshi == wuci )
				{
					...
					Test_quanju_canshu_L.time_softdelay_ms = 0;
					//重置单步超时时间 10秒
					Test_quanju_canshu_L.danbu_chaoshishijian_ms = 35000;//第二次查询结果时间10s
		...
		case w_chaxun_jieguo_second: //第二次查询只负责校验有磁计量和阀门到位
			if(test_xieyi_jilu_Rec == w_get_test_zhuanyong) 
			{
				test_xieyi_jilu_Rec = No_Receive;
					...
					if(Test_jiejuo_jilu.jiliang_fangshi == youci)
					{
					Test_liucheng_L = w_jiliang_qiangci_CHK_1;  //如果有阀门，则Test_liucheng_L = w_famenceshi
					}
					if(Test_jiejuo_jilu.jiliang_fangshi == wuci)
					{
		...
		else
		{
				//发送协议
				send_chaxun_xieyi();
			Test_quanju_canshu_L.time_softdelay_ms = 1200;
		}
		break;
			//计量检测
