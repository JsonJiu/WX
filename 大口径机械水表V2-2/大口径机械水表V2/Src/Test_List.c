	...
	switch(Test_liucheng_L)
	{
		...
		case w_zhudong_shangg:
	      ...
	      Test_quanju_canshu_L.danbu_chaoshishijian_ms = 20000;
				...
				{
					SENSOR_T_Low();   //因为水表板需要中断才会触发，所以要先改变一下电平
					SENSOR_P_High();
					SENSOR_D_Low();
				}
			}
			...
			break;
		case w_yali_jiance:
			if(EXTIFlag == 1 && test_xieyi_jilu_Rec == w_get_yali)    
			{
				//测试合格，进入下一步
				Test_quanju_canshu_L.time_softdelay_ms = 0;
				//重置单步超时时间
		...
		case w_chaxun_jieguo_first:
			if(test_xieyi_jilu_Rec == w_get_test_zhuanyong) 
			{
				test_xieyi_jilu_Rec = No_Receive;
					...
					//这里第一次测有磁计量
					if(Test_linshi_cunchushuju_L.L_ciganrao_CHK == 1 && Test_linshi_cunchushuju_L.L_jilianghuoer1 == 0 && Test_linshi_cunchushuju_L.L_jilianghuoer2 == 1)//有磁是和实际电平相反，因为协议给到的是状态
					{
						Test_jiejuo_jilu.jiliang_1_1 = 1;  //表示有磁计量高电平检测正常
						//有磁计量电平切换
						SENSOR_P_Low();
						SENSOR_D_High();
						SENSOR_T_High();
		...
		case w_chaxun_jieguo_second: //第二次查询只负责校验有磁计量和阀门到位
			if(test_xieyi_jilu_Rec == w_get_test_zhuanyong) 
			{
				test_xieyi_jilu_Rec = No_Receive;
					...
					if(Test_linshi_cunchushuju_L.L_ciganrao_CHK == 0 && Test_linshi_cunchushuju_L.L_jilianghuoer1 == 1 && Test_linshi_cunchushuju_L.L_jilianghuoer2 == 0)
					{
					...
					Test_quanju_canshu_L.time_softdelay_ms = 0;
					//重置单步超时时间 20秒
					Test_quanju_canshu_L.danbu_chaoshishijian_ms = 20000;//阀门测试时间20s
					Test_liucheng_L = w_guanbihongwai;  //如果有阀门，则Test_liucheng_L = w_famenceshi
			}
			else
			{
				//发送协议
			...
			break;
	  //这板子有阀门
		case w_guanbihongwai:
			if(test_xieyi_jilu_Rec == w_guanbihongwai_L)
			{
				test_xieyi_jilu_Rec = No_Receive;
				guanhongwai_jishu = 0;
				//重置单步超时时间 10秒
...
//				RS2103_POWER_OFF();
			...
			else
			{
				guan_hongwai_xieyi();
				guanhongwai_jishu ++;
				if(guanhongwai_jishu == 2)
				{
					test_xieyi_jilu_Rec = w_guanbihongwai_L;
				}
				//等待5秒自动重发
